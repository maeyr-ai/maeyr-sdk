...
class MaeyrAuthError(RuntimeError):
...
class MaeyrAuth:
    ...
    ``MAEYR_AUTH_ENABLED_METHODS``. All methods declared for this agent
    (including those still missing secrets) are listed in
    ``MAEYR_AUTH_CONFIGURED_METHODS``.
    ...
    _ENABLED_ENV_KEY = "MAEYR_AUTH_ENABLED_METHODS"
    _CONFIGURED_ENV_KEY = "MAEYR_AUTH_CONFIGURED_METHODS"
    ...
    def require_param(method_id: str, param_name: str) -> str:
        ...
        """
        value = MaeyrAuth.get_param(method_id, param_name)
        if value is None:
            raise MaeyrAuthError(
    ...
    def preferred_method(*candidates: str) -> Optional[str]:
        ...
        one at runtime in a fixed preference order, e.g.::
            ...
            if method == "oauth_client":
                ...
        """
        enabled = MaeyrAuth.get_enabled_methods()
    ...
    def require_method(*candidates: str) -> str:
        """Like ``preferred_method`` but raises ``MaeyrAuthError`` if none match.

        With no candidates, returns the first enabled method or raises if none
        are enabled at all.
        """
        if candidates:
            chosen = MaeyrAuth.preferred_method(*candidates)
            if chosen is None:
                raise MaeyrAuthError(
                    "None of the requested auth methods are configured: {}. "
                    "Enabled methods: {}.".format(list(candidates), MaeyrAuth.get_enabled_methods())
                )
            return chosen
        enabled = MaeyrAuth.get_enabled_methods()
        if not enabled:
            raise MaeyrAuthError(
                ...
                "method on this agent to use MaeyrAuth.require_method()."
            )
        return enabled[0]
