"""Proxy stdio MCP clients to the hosted Maeyr MCP gateway."""
...
def resolve_gateway_url(
    ...
    * ``gateway_url`` — full override (env: ``MAEYR_MCP_GATEWAY_URL``)
...
def resolve_mcp_token(explicit: Optional[str] = None) -> str:
    ...
    if not token:
        raise ValueError(
            ...
            "(create tokens in the Maeyr console → MCP Tokens)"
        )
    return token
...
async def run_stdio_gateway_proxy(
    *,
    ...
    headers = {
        ...
        "X-MCP-Client": "maeyr-mcp-bridge",
        "User-Agent": f"maeyr-mcp-bridge/{__version__}",
    }
    ...
    async with httpx.AsyncClient(headers=headers, timeout=timeout, follow_redirects=True) as http:
        async with streamable_http_client(gateway_url, http_client=http) as (
            ...
            async with ClientSession(
                remote_read,
                remote_write,
                client_info=types.Implementation(name="maeyr-mcp-bridge", version=__version__),
            ) as upstream:
                init_result = await upstream.initialize()
                logger.info("Connected to Maeyr MCP gateway: %s", gateway_url)
