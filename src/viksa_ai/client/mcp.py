"""Call Viksa agent tools via the hosted MCP gateway (not pulse directly)."""
...
class McpClient:
    """
    Execute Viksa agent tools through **mcp-gateway-service**.

    Use this (or Cursor/Claude MCP URL config) for agent tool calls — not
    ``ViksaClient.pulse.execute``, which bypasses the MCP gateway and breaks
    ...
    def _headers(self) -> Dict[str, str]:
        return {
            ...
            "User-Agent": f"viksa-ai/{__version__}",
        }
