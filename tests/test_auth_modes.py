...
from viksa_ai.models.auth import KeyValidationResponse, TokenResponse


def test_from_api_key_sets_bearer_and_kind():
    client = ViksaClient.from_api_key("vk_test_key", base_url="https://api.example.com")
...
def test_from_api_key_validate_fills_tenant():
    with patch(
        "viksa_ai.client.auth.AuthClient.validate_api_key_sync",
        ...
        client = ViksaClient.from_api_key(
            "vk_test_key",
...
def test_from_api_key_validate_invalid():
    with patch(
        ...
        with pytest.raises(ValueError, match="revoked"):
            ViksaClient.from_api_key("bad", validate=True)
...
async def test_from_login():
    ...
    with patch("viksa_ai.client.auth.AuthClient.login", new=fake_login):
        client = await ViksaClient.from_login(
            "user@example.com",
            "secret",
            base_url="https://api.test",
        )
...
def test_from_login_sync():
    ...
    with patch("viksa_ai.client.auth.AuthClient.login_sync", new=fake_login):
        client = ViksaClient.from_login_sync("a@b.com", "pw", base_url="https://api.test")
    assert client.access_token == "jwt-1"
    client.close()
...
def test_login_omits_empty_bearer_header():
    ...
    with httpx.Client(transport=transport, base_url="https://api.test") as http:
        client = ViksaClient(access_token="", base_url="https://api.test")
...
def test_from_env_requires_credential(monkeypatch):
    ...
    with pytest.raises(ValueError, match="VIKSA_API_KEY"):
        ViksaClient.from_env()
