...
from maeyr_platform.directory.prompt_cache import _client
from maeyr_platform.observability.logging import get_logger

logger = get_logger("[maeyr_platform.directory.invoker_cache]")
...
local envelope = cjson.encode({
  __maeyr_cache_generation = tonumber(current),
...
async def _get_cache_value(
    ...
    *,
    ...
    if not isinstance(data, dict):
        return None
    stored_generation = data.get("__maeyr_cache_generation")
