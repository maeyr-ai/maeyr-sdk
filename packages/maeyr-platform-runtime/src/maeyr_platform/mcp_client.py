...
_CLIENT_PATTERNS: tuple[tuple[re.Pattern[str], str, str], ...] = (
    ...
    (
        re.compile(r"maeyr[- ]?mcp[- ]?bridge", re.I),
        "maeyr_mcp_bridge",
        "Maeyr MCP Bridge",
    ),
    (re.compile(r"maeyr[- ]?ai|maeyr[- ]?sdk", re.I), "maeyr_sdk", "Maeyr SDK"),
)
...
def resolve_mcp_client(headers: Mapping[str, str]) -> MCPClientInfo:
    ...
    override = _header(headers, "x-mcp-client") or _header(headers, "x-maeyr-mcp-client")
