...
from viksa_platform.tracing import transport
from viksa_platform.tracing.constants import REDIS_PROCESSING_QUEUE_KEY
from viksa_platform.tracing.remote_recorder import RemoteTraceRecorder
...
async def test_acknowledged_push_returns_after_remote_acceptance() -> None:
    ...
    with (
        ...
        patch(
            "viksa_platform.tracing.transport.enqueue_span",
...
async def test_background_http_failure_moves_unresolved_span_to_dead_letter() -> None:
    ...
    with (
        ...
        patch(
            "viksa_platform.tracing.transport.re_enqueue_spans",
